...
def displacement(
    ...
    """
    Displacement components produced by pore-pressure variations in
    right-rectangular prisms

    The displacement components are computed by integrating the solution
    ...
    field : str
        Displacement component to be computed.
    ...
    -------
    result : array
        Displacement component generated by the prisms at the computation
        points.
...
@jit(nopython=True)
def kernel_d_x1(y, x, z, cz, yp, xp, zp):
    """
    Kernel for x-component of displacement in the infinite space domain
    (first system) computed with expressions given by Nagy et al. (2000)
    """
    Y = y - yp
    X = x - xp
    Z = z - zp
...
@jit(nopython=True)
def kernel_d_y1(y, x, z, cz, yp, xp, zp):
    """
...
@jit(nopython=True)
def kernel_d_z1(y, x, z, cz, yp, xp, zp):
    """
...
@jit(nopython=True)
def kernel_d_x2(y, x, z, cz, yp, xp, zp):
    """
    Kernel for x-component of displacement in the semi-infinite space domain
    (second system) computed with expressions given by Nagy et al. (2000)
    """
    Y = y - yp
    X = x - xp
    Z = z - zp - 2 * cz
...
@jit(nopython=True)
def kernel_d_y2(y, x, z, cz, yp, xp, zp):
    """
...
@jit(nopython=True)
def kernel_d_z2(y, x, z, cz, yp, xp, zp):
    """
...
@jit(nopython=True)
def kernel_d_xz2(y, x, z, cz, yp, xp, zp):
    """
...
@jit(nopython=True)
def kernel_d_yz2(y, x, z, cz, yp, xp, zp):
    """
...
@jit(nopython=True)
def kernel_d_zz2(y, x, z, cz, yp, xp, zp):
    """
