...
at the $i$th coordinates  $(x_i$, $y_i, z_i)$, we integrate the solution deduced for a single nucleus of strain (equation \ref{eq:nucleus-ui_total})
...
The integral depending on ${\bf{u_{1}}}_{i}$,
the displacement field due to a single nucleus in the infinite space (equations \ref{eq:nucleus-u1} and  \ref{eq:nucleus-u1-vector}), is conducted over the volume of the $j$th prism. The integral depending on ${\bf{u_{2}}}_{i}$,
...
Finally, the displacement field at the coordinates $x_i$, $y_i$  and $z_i$ due to the pressure change in the whole reservoir is defined as the sum of the displacements yielded by each prism with constant pressure $\Delta p_{j}$:
\begin{equation}
...
By substituting equations \ref{eq:nucleus-u1-vector} and \ref{eq:nucleus-u2-vector} into equation \ref{eq:nucleus-ui_M_prisms}, we obtain the components of the displacement field ${\bf{\tilde{u}}}_{i} (x_i,y_i,z_i)$ due to the pressure change in the whole reservoir, i.e.:
\begin{equation}
...
\label{eq:u_til_z}
\end{equation}
where $\alpha = x, y$, $dv_j$ is the $j$th element of volume of the $j$th prism whose volume is $v_j$.
