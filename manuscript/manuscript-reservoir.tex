...
\end{equation}
where both integrals are conducted with respect to the variables $(x^{\prime}_{j}, y^{\prime}_{j}, z^{\prime}_{j})$, but the first is conducted over the volume ${v_{1}}_{j}$ of the $j$th prism and the second is conducted over the volume 
...
top and bottom borders given by $z_1 - 2 \, z_c$ and $z_2 - 2 \, z_c$, where 
$z_c = \frac{1}{2}(z_1 + z_2)$ is the center depth of the $j$th prism.
By substituting equations \ref{eq:nucleus-u1-vector} and \ref{eq:nucleus-u2-vector} into equation \ref{eq:nucleus-ui_jth_prism}, we obtain
\begin{equation}
...
$(x_i, y_i, z_i)$, ${R_1} = \sqrt{x^{2} + y^{2} + z^{2}}$, and
\begin{equation}
\begin{array}{ll}
{X_1}_{j} &= x_i - {x_1}_{j} \\
{X_2}_{j} &= x_i - {x_2}_{j} \\
