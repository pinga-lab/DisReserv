...
is the correction of the displacements considering a semi-space (image nucleus solution).
...
$(x_{i}, y_{i}, z_{i})$, 
$\Delta p_j$  is the pressure contrast of the $j$th nucleus, $dv_j$ is an 
infinitesimal element of volume of the $j$th nucleus, $A$ is the constant
\begin{equation}
A = - \frac{C_m E}{4 \pi (1 + \nu)} \: ,
\label{eq:nucleus-A}
\end{equation}
...
For convenience, the $x-$, $y-$ and $z-$components of the vectors ${\bf{u_{1}}}_{i}$ (equation \ref{eq:nucleus-u1}) and ${\bf{u_{2}}}_{i}$ (equation \ref{eq:nucleus-u2}) 
...
To calculate the displacement field produced by the $jth$ prism
...
\int_{y_1}^{y_2} \:\: 
\int_{x_1}^{x_2} 
{\bf{u_{1}}}_{ij} \: \:  dx^{\prime}_j dy^{\prime}_j dz^{\prime}_j 
\: + \:
\int_{z_1 - 2 \, z_c}^{z_2 - 2 \, z_c} \:\:
\int_{y_1}^{y_2} \:\: 
\int_{x_1}^{x_2} 
{\bf{u_{2}}}_{ij} \:\:  dx^{\prime}_j dy^{\prime}_j dz^{\prime}_j \: ,
\label{eq:nucleus-ui_jth_prism}
\end{equation}
where the limits of the integrals are defined by the borders and the center 
of the $j$th prism in the following way: $x_1$ and $x_2$ are their south and north borders; $y_1$ and  $y_2$ are their  west and east borders; $z_1$ and $z_2$ are their depths to the top and bottom; and $z_c = \frac{1}{2}(z_1 + z_2)$ is its center depth. 
In equation \ref{eq:nucleus-ui_jth_prism}, the integrations are conducted with respect to the variables $(x^{\prime}_{j}, y^{\prime}_{j}, z^{\prime}_{j})$. 
The integral depending on ${\bf{u_{1}}}_{ij}$,
the displacement field due to a single nucleus in the infinite space (equations \ref{eq:nucleus-u1} and  \ref{eq:nucleus-u1-vector}), is conducted over the volume of the $j$th prism. The integral depending on ${\bf{u_{2}}}_{ij}$,
the displacement field due to the image nucleus (equations \ref{eq:nucleus-u2} and  \ref{eq:nucleus-u2-vector}), is conducted over the volume of a different prism
symmetrically positioned above the free surface and conveniently called "image prism".

By substituting equations \ref{eq:nucleus-u1-vector} and \ref{eq:nucleus-u2-vector} into equation \ref{eq:nucleus-ui_jth_prism}, we obtain the components of the displacement vector ${\bf{\tilde{u}}}_{ij}$ due to the pressure change in the $j$th prism, i.e.:
\begin{equation}
{\tilde{u}}_{{ij}_\alpha} = \frac{A  \: (1 + \nu)}{E} 
\: \Delta p_{j}  \: \left[ \int\int\limits_{v_j}\int 
...
\label{eq:u_til_z}
\end{equation}
where $\alpha = x, y$ and $dv_j$ is the $j$th element of volume of the $j$th prism 
whose volume is $v_j$.
In the right-hand side of equations \ref{eq:u_til_alpha} and \ref{eq:u_til_z}, the three integrals have the same form of derivatives of the gravitational field produced by the 
$j$th prism. 
...
The similarity between the displacement fields due to a volume source in a half-space and the gravitational field allows the use of closed expressions of the gravitational potential and its derivatives produced by the 3D right rectangular prism derived by \cite{Nagyetal2000} and (\citeyear{Nagyetal2002}).  
...
% Displacement produced by the whole reservoir 
Finally, the displacement field at the point $(x_i, y_i, z_i)$ due to the pressure change in the whole reservoir is defined as the sum of the displacements 
${\bf{u}}_{ij}$ (equation \ref{eq:nucleus-ui_jth_prism}) yielded by each prism 
with constant pressure $\Delta p_{j}$:
\begin{equation}
{\bf{\tilde{u}}}_{ij} = \sum_{j=1}^{M} {\bf{u}}_{ij} \: ,
